...
# @app.route('/api/get-chosen-song-give-reccomended-songs-2', methods=['GET'])
# def get_chosen_song_give_reccomended_songs_2():
#   song_info = request.get_json()

#   df = pd.DataFrame.from_dict([song_info])
#   genre_categories = predict_genre(df)

#   songs = s.get_recommendations_from_genre(genres)
...
#     track_id = track['id']
#     explicit = track['explicit']
#     danceability, energy, key, loudness, mode, speechiness, acousticness, instrumentalness, liveness, valence, tempo = s.get_track_features(
#       track_id)
#     text = chat_gpt.discuss_songs(track_name)
